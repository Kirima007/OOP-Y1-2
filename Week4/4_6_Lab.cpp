...
class labtop : public computing_device , public telephone{
    public:
        ...
        labtop (string _name , processor _cpu ,float _ram_capacity , input_device _keybord ,string _os){
            name = _name;
            cpu = _cpu;
        ...
        void print(){
            cout << name << " has [" << processing_power() << "] power with [" << input.name << "] input " << endl;
        }
};
...
class smartphone : public computing_device ,public telephone{
    ...
    smartphone (string _name ,processor _cpu ,float _ram_capacity,input_device _touch ,string _network){
        name = _name;
        cpu = _cpu;
        ram_capacity = _ram_capacity;
        input = _touch;
    ...
    void print(){
        if (cpu.brand == "Apple ")cout << name << " using [IOS] with ["<< network <<"] and running at ["<<processing_power()<<"] power"<<endl;
        else cout << name << " using [Android] with ["<< network <<"] and running at ["<<processing_power()<<"] power"<<endl;
    }
...
int main()
{
    ...
    //ชื่อรุ่น str , process , ram , keybord , os str
    ...
    //name pro ram input_device network
